...
class CommentAdapter:
    ...
    def cluster(self, comments: str, competitor: str) -> List[Evidence]:
        """
        将长评论聚类为多条 user_feedback Evidence
        ...
        Returns:
            多条 user_feedback 维度的 Evidence
        """
        if not comments or len(comments.strip()) < 10:
            logger.warning("评论内容过短，返回单条 Evidence")
            return self._create_single_evidence(comments, competitor)
        
        try:
            themes = self._extract_themes_with_llm(comments)
            return self._create_evidence_from_themes(themes, competitor)
        except LLMNotConfiguredError:
            logger.info("LLM 未配置，使用关键词聚类")
            themes = self._extract_themes_by_keywords(comments)
            return self._create_evidence_from_themes(themes, competitor)
    ...
    def _extract_themes_with_llm(self, comments: str) -> List[Dict[str, Any]]:
        """使用 LLM 提取评论主题"""
        prompt = self._build_cluster_prompt(comments)
    ...
    def _build_cluster_prompt(self, comments: str) -> str:
        """构建聚类 prompt"""
        prompt_parts = [
            "请从以下用户评论中提取关键主题，每个主题包含：",
            "- theme: 主题名称（简洁，如模板同质化、定价偏高）",
    ...
    def _extract_themes_by_keywords(self, comments: str) -> List[Dict[str, Any]]:
    ...
    def _create_evidence_from_themes(self, themes: List[Dict[str, Any]], competitor: str) -> List[Evidence]:
        """从主题列表创建 Evidence"""
        evidences = []
    ...
    def _create_single_evidence(self, comments: str, competitor: str) -> List[Evidence]:
