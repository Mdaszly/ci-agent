...
_task_id_seq = 0
_task_id_last_stamp = ""
_task_id_suffix_map: dict[str, str] = {}
...
def _business_suffix(source: str, stage: str | None = None, object_id: str | None = None) -> str:
    parts = [_slug_token(source)]
    if stage:
        parts.append(_slug_token(stage))
    if object_id:
        parts.append(_slug_token(object_id))
    return "_".join(parts)


def register_task_alias(task_id: str, *, source: str, stage: str | None = None, object_id: str | None = None) -> str:
    suffix = _business_suffix(source, stage, object_id)
    _task_id_suffix_map[task_id] = suffix
    return task_id


def resolve_task_alias(task_id: str) -> str:
    return _task_id_suffix_map.get(task_id, task_id)


def new_task_id(prefix: str = "task", *, source: str | None = None, stage: str | None = None, object_id: str | None = None) -> str:
    global _task_id_seq, _task_id_last_stamp

    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    with _task_id_lock:
        if stamp != _task_id_last_stamp:
            _task_id_last_stamp = stamp
            _task_id_seq = 0
        else:
            _task_id_seq += 1
        seq = _task_id_seq
    if source or stage or object_id:
        suffix = _business_suffix(source or prefix, stage, object_id)
        return f"{prefix}_{stamp}_{seq:02d}_{suffix}"
    return f"{prefix}_{stamp}_{seq:02d}"


def build_task_observable_id(task: "TaskRecord") -> str:
...
class BadCase(BaseModel):
    id: str = Field(default_factory=lambda: new_id("badcase"))
